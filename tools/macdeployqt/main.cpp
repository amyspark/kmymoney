...
int main(int argc, char **argv)
{
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        } else if (argument.startsWith(QByteArray("-background"))) {
          LogDebug() << "Argument found:" << argument;
          const int index = argument.indexOf('=');
          if (index == -1)
            LogError() << "Missing background file path";
          else
            backgroundFile = argument.mid(index + 1);
        } else if (argument.startsWith(QByteArray("-script"))) {
          LogDebug() << "Argument found:" << argument;
          const int index = argument.indexOf('=');
          if (index == -1)
            LogError() << "Missing path to AppleScript script";
          else
            script = argument.mid(index + 1);
        } else if (argument.startsWith(QByteArray("-icon"))) {
          LogDebug() << "Argument found:" << argument;
          const int index = argument.indexOf('=');
          if (index == -1)
            LogError() << "Missing path to ICNS icon";
          else
            icon = argument.mid(index + 1);
    ...
    if (!qmlDirs.isEmpty()) {
        ...
        const QSet<QString> setFrameworks{
            deploymentInfo.deployedFrameworks.cbegin(),
            deploymentInfo.deployedFrameworks.cend(),
        };
        
        deploymentInfo.deployedFrameworks = QStringList{setFrameworks.cbegin(), setFrameworks.cend()
        };
    }
    ...
    if (dmg) {
        LogNormal();
        createDiskImage(appBundlePath, filesystem, backgroundFile, icon, script);
    }
