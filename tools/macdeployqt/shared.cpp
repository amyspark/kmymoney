...
QString getBundleExecutable(const QString &infoPlistPath, const QString &appBundleName)
{
    const QByteArray contents = [&]() {
        QFile infoPlist(infoPlistPath);
        infoPlist.open(QIODevice::ReadOnly);
        return infoPlist.readAll();
    }();
    CFDataRef xmlData = CFDataCreateWithBytesNoCopy(NULL, reinterpret_cast<const UInt8 *>(contents.constData()), contents.length(), kCFAllocatorNull);
    if (!xmlData) {
        LogError() << "Cannot parse the Info.plist file (internal Core Foundation error)";
        return appBundleName;
    }
    CFErrorRef error = nullptr;
    CFPropertyListRef dictionary = CFPropertyListCreateWithData(NULL, xmlData, kCFPropertyListImmutable, NULL, &error);
    if (error) {
        CFStringRef msg = CFErrorCopyDescription(error);
        LogError() << "Cannot parse the Info.plist file:" << msg;
        CFRelease(error);
        CFRelease(xmlData);
        return appBundleName;
    }
    if (!dictionary) {
        LogError() << "Cannot create the CFPropertyList (internal Core Foundation error)";
        return appBundleName;
    }
    const void *appBundleExecutable = nullptr;
    const bool appExecutableExists = CFDictionaryGetValueIfPresent((CFDictionaryRef)dictionary, CFSTR("CFBundleExecutable"), &appBundleExecutable);
    if (!appExecutableExists) {
        CFRelease(dictionary);
        CFRelease(xmlData);
        LogError() << "CFBundleExecutable in Info.plist missing";
        return appBundleName;
    }

    QString appExecutableName = QString::fromCFString((CFStringRef)appBundleExecutable);
    CFRelease(appBundleExecutable);
    //CFRelease(dictionary); // CRASH?
    CFRelease(xmlData);
                                  ...
                                  const QList<QString> &additionalBinariesContainingRpaths)
{
    ...
    for (const DylibInfo &info: dependencies) {
        ...
        if (trimmedLine.startsWith("@executable_path/")) {
            ...
            // If a framework was relocated by a library prior to the
            // current one, it'll have been reassigned from rpath
            // to load_path. Adjust.
            bool resolved = false;
            rpaths = {
            ...
            for (const QString &rpath: rpaths) {
                QString binary = QDir::cleanPath(rpath + "/" + trimmedLine.mid(QStringLiteral("@loader_path/").length()));
...
void changeInstallName(const QString &bundlePath, const FrameworkInfo &framework, const QStringList &binaryPaths, bool useLoaderPath)
{
    ...
    for (const QString &binary: binaryPaths) {
        ...
        const QFileInfo fileInfo(framework.installName);
...
void deployRPaths(const QString &bundlePath, const QSet<QString> &rpaths, const QString &binaryPath, bool useLoaderPath)
{
    ...
    QStringList args;
    for (const QString &rpath: getBinaryRPaths(binaryPath, false)) {
                             ...
                             QList<QString> additionalBinariesContainingRpaths)
{
    ...
    const auto appBundle = QFileInfo(appBundlePath);
    const QString appBundleAbsolutePath = appBundle.absoluteFilePath();
    ...
    // Remember: foundRootBinaries is a stack!! - Amyspark
    const QString appBundleInfoPlist = appBundleAbsolutePath + "/Contents/info.plist";
    const QString appBundleExecutable = getBundleExecutable(appBundleInfoPlist, appBundle.baseName());
    if (foundRootBinaries.contains(appBundleExecutable)) {
        LogWarning() << "Found binaries apart from the main executable, sorting it out!";
        foundRootBinaries.removeAll(appBundleExecutable);
        foundRootBinaries.prepend(appBundleExecutable);
        LogWarning() << "Binaries to sign (in reverse order):" << foundRootBinaries;
    }
    ...
    LogWarning() << "Pending binaries to sign (in this order):" << pendingBinaries;
    while (!pendingBinaries.isEmpty()) {
        ...
        const auto dependencies = [&]() {
            const auto deps = getBinaryDependencies(rootBinariesPath, binary, additionalBinariesContainingRpaths);
            return QSet(deps.begin(), deps.end())
                .subtract(signedBinaries)
                .subtract(pendingBinariesSet);
        }();

        // LogNormal() << "=== Dependencies to sign for:" << binary << "===";
        // LogNormal() << dependencies;
        // // This binary is egregious because it relocates a library
        // // prior to the one that'll drive its signing.
        // if (binary.endsWith("qcocoa.dylib")) {
        //     LogWarning() << "=== Signed? " << signedBinaries;
...
void createDiskImage(const QString &appBundlePath, const QString &filesystemType, const QString &backgroundFile, const QString &icns, const QString &script)
{
    ...
    {
      LogNormal() << "Preparing temporary image root in" << tempDir.path();

      QStringList options{"-R", appBundlePath, tempDir.path()};

      QProcess cp;
      cp.start("cp", options);
      cp.waitForFinished(-1);
      if (cp.exitCode() != 0) {
        LogError() << "Copy to temporary root error:"
                   << cp.readAllStandardError();
      }
    }

    {
      LogNormal() << "Adding alias to /Applications";

      const QStringList options{"-s", "/Applications",
                                tempDir.filePath("Applications")};

      QProcess mkalias;
      mkalias.start("ln", options);
      mkalias.waitForFinished(-1);
      if (mkalias.exitCode() != 0) {
        LogError() << "/Applications link error:"
                   << mkalias.readAllStandardError();
      }
    }

    // If background file: format must be udrw
    const bool needsRemount = !backgroundFile.isEmpty() || !icns.isEmpty();

    QString tempImageFormat = "ULMO";

    if (needsRemount) {
      // add a 10MB placeholder file to the image

      QFile f(tempDir.filePath("padding"));
      if (!f.open(QFile::WriteOnly)) {
        LogError() << "Failed preparing the padding";
      }

      const QByteArray b(1024 * 1024 * 10, '\0');
      if (f.write(b) != b.size()) {
        LogError() << "Failed writing the padding";
      }

      tempImageFormat = "UDRW";
    }

    const QString tempImage = QDir(QDir::currentPath()).absoluteFilePath(QStringLiteral("temp.dmg"));

    {
      // Prepare the temporary image
      const QStringList options{
          QStringLiteral("create"),     tempImage,
          QStringLiteral("-ov"),
          QStringLiteral("-srcfolder"), tempDir.path(),
          QStringLiteral("-format"),    tempImageFormat,
          QStringLiteral("-fs"),        filesystemType,
          QStringLiteral("-volname"),   appBaseName};

      QProcess hdiutil;
      hdiutil.start("hdiutil", options);
      hdiutil.waitForFinished(-1);
      if (hdiutil.exitCode() != 0) {
        LogError() << "Temporary bundle creation error:"
                   << hdiutil.readAllStandardError();
      }
    }

    if (needsRemount) {
        const QString attachOutput = [&]() {
            QProcess hdiutil;
            hdiutil.start("hdiutil", QStringList{
                QStringLiteral("attach"),
                tempImage,
            });
            hdiutil.waitForFinished(-1);
            if (hdiutil.exitCode() != 0) {
                LogError() << "Mounting temporary bundle failed:"
                        << hdiutil.readAllStandardError();
            }
            return QString::fromUtf8(hdiutil.readAllStandardOutput());
        }();

        const QRegularExpression mountpoint(".*(/Volumes/[^\n]+)\n.*");
        const QRegularExpressionMatch match = mountpoint.match(attachOutput);
        const QString tempMount = match.captured(1);
        const QString tempVolumeName = tempMount.split('/')[2];

        LogNormal() << "Mounted temporary image at" << tempMount;

        QFile f(QDir(tempMount).absoluteFilePath("padding"));
        if (!f.remove()) {
            LogError() << "Error removing padding file" << f.fileName();
        }

        if (!icns.isEmpty()) {
            if (!applyIconToFolder(tempMount, icns)) {
                LogError() << "Error applying icon to temporary disk image.";
            }
        }

        if (!backgroundFile.isEmpty()) {
            QFileInfo f(backgroundFile);
            QFile src(backgroundFile);
            QDir tmp(tempMount);
            tmp.mkdir(".background");
            tmp.cd(".background");
            LogNormal() << "Copying" << src.fileName() << "to" << tmp.absolutePath();
            src.copy(tmp.absoluteFilePath(f.fileName()));
        }

        // configure and run osascript with the CMake script
        if (!script.isEmpty()) {
            LogNormal() << "Running AppleScript" << script;
            QProcess osascript;
            const QFile sc(script);
            osascript.start("osascript", QStringList{script, tempVolumeName});
            osascript.waitForFinished(-1);
            if (osascript.exitCode() != 0) {
                LogError() << "Error executing custom script on disk image:"
                        << osascript.arguments() 
                        << osascript.readAllStandardError();
            }
        }

        // Finished modifications, unmount
        LogNormal() << "Unmounting" << tempMount;
        QProcess hdiutil;
        hdiutil.start("hdiutil", QStringList{
        ...
        hdiutil.waitForFinished(-1);
        if (hdiutil.exitCode() != 0) {
            LogError() << "Unmounting temporary bundle failed:"
                    << hdiutil.readAllStandardError();
        }
    }

    // More dmg options can be found in the hdiutil man page.
    LogNormal() << "Finalizing" << dmgName;
    const QStringList options = {
        QStringLiteral("convert"), QStringLiteral("temp.dmg"),
        QStringLiteral("-format"), QStringLiteral("ULMO"), // Use LZMA (macOS 10.15+)
        QStringLiteral("-o"), dmgName
    };
